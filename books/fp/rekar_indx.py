index = {
    "2plus3": {
        "dmath": {
            """<mn>2</mn>
    <mo>+</mo>
    <mn>3</mn>
    <mo>=</mo>
    <mn>5</mn>
    """},
    },
    "3plus2": {
        "dmath": {
            """<mn>3</mn>
    <mo>+</mo>
    <mn>2</mn>
    <mo>=</mo>
    <mn>5</mn>
    """},
    },
    "plussym": "<span class='symbol'><math class='inline-math'><mo>+</mo></math></span>",
    "2plus3inline": {
        "math": {
            """<mn>2</mn>
    <mo>+</mo>
    <mn>3</mn>
    <mo>=</mo>
    <mn>5</mn>
    """},
    },
    "pluscommutative": {
        "dmath": """<mtable displaystyle="true" class="tml-jot">
    <mtr>
      <mtd class="tml-right" style="padding-left:0em;padding-right:0em;">
        <mrow>
          <mn>2</mn>
          <mo>+</mo>
          <mn>5</mn>
        </mrow>
      </mtd>
      <mtd class="tml-left" style="padding-left:0em;padding-right:0em;">
        <mrow>
          <mo>=</mo>
          <mn>7</mn>
          <mo>=</mo>
          <mn>5</mn>
          <mo>+</mo>
          <mn>2</mn>
        </mrow>
      </mtd>
    </mtr>
    <mtr>
      <mtd class="tml-right" style="padding-left:0em;padding-right:0em;">
        <mrow>
          <mn>6</mn>
          <mo>+</mo>
          <mn>3</mn>
        </mrow>
      </mtd>
      <mtd class="tml-left" style="padding-left:0em;padding-right:0em;">
        <mrow>
          <mo>=</mo>
          <mn>9</mn>
          <mo>=</mo>
          <mn>3</mn>
          <mo>+</mo>
          <mn>6</mn>
        </mrow>
      </mtd>
    </mtr>
  </mtable>
  """,
    },
    "2plus7": {
        "dmath": {
            """<mn>2</mn>
    <mo>+</mo>
    <mn>7</mn>
    <mo>=</mo>
    <mn>9</mn>
    """},
    },
    "4plus11": {
        "dmath": {
            """<mn>4</mn>
    <mo>+</mo>
    <mn>11</mn>
    <mo>=</mo>
    <mn>15</mn>
    """},
    },
    "symeq": "<span class='symbol'><math class='inline-math'><mo>=</mo></math></span>",
    "5min3": {
        "dmath": "<mn>5</mn><mo>−</mo><mn clas='minus'>3</mn><mo>=</mo><mn>2</mn>",
    },
    "5min3lang": {
        "dmath": "<mn>5</mn><mo>−</mo><mn>3</mn><mo>=</mo><mn>2</mn>",
    },    
    "5min3inline": {
        "math": "<mn>5</mn><mo>−</mo><mn>3</mn><mo>=</mo><mn>2</mn>",
    },       
    "minsym": "<span class='symbol'><math class='inline-math'><mo>−</mo></math></span>",
    "7min7": {
        "math": "<mn>7</mn><mo>−</mo><mn>7</mn><mo>=</mo><mn>0</mn>",
    },        
    "19min19": {
        "math": "<mn>19</mn><mo>−</mo><mn>19</mn><mo>=</mo><mn>0</mn>",
    },
    "6min4": {
        "dmath": "<mn>19</mn><mo>−</mo><mn>19</mn><mo>=</mo><mn>0</mn>",
    }, 
    "12min5": {
        "dmath": "<mn>19</mn><mo>−</mo><mn>19</mn><mo>=</mo><mn>0</mn>",
    }, 
    "mincncla": {
        "dmath": "<mn>4</mn><mo>+</mo><mn>3</mn><mo>−</mo><mn>4</mn><mo>+</mo><mn>4</mn><mo>+</mo><mn>7</mn><mo>−</mo><mn>4</mn><mo>+</mo><mn>1</mn>",
    }, 
    "mincnclb": {
        "dmath": "<mn>3</mn><mo>+</mo><mn>7</mn><mo>+</mo><mn>1</mn>",
    },
    "arrow": {
        "footnote": "I figurer med tallinjer vil rødfargede piler indikere at man starter ved pilspissen og vandrer til andre enden."
    },
    "5eq2plus3": {
        "math": "<mn>5</mn><mo>=</mo><mn>2</mn><mo>+</mo><mn>3</mn>",
    },
    "5eq1plus4": {
        "math": "<mn>5</mn><mo>=</mo><mn>2</mn><mo>+</mo><mn>3</mn>",
    },
    "firsttimes": {
        "dmath": """<mtable>
        <mtr>
          <mtd>
            <mn>4</mn><mo>+</mo><mn>4</mn><mo>+</mo><mn>4</mn>
          </mtd>
          <mtd> <mo>=</mo></mtd>
          <mtd><mn>4</mn><mo>⋅</mo><mn>3</mn></mtd>
        </mtr>
        <mtr>
          <mtd></mtd>
        </mtr>
        <mtr>
          <mtd>
            <mn>8</mn><mo>+</mo><mn>8</mn>
          </mtd>
          <mtd> <mo>=</mo></mtd>
          <mtd><mn>8</mn><mo>⋅</mo><mn>2</mn></mtd>
        </mtr>
        <mtr>
          <mtd></mtd>
        </mtr>
        <mtr>
          <mtd><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>1</mn><mo>⋅</mo><mn>5</mn></mtd>
        </mtr>
      </mtable>""",
    },
    "timessym": "<span class='symbol'><math class='inline-math'><mo>⋅</mo></math></span>",
    "4times12": {
        "dmath": "<mn>4</mn><mo>⋅</mo> <mn>3</mn><mo>=</mo><mn>12</mn>",
    },
    "4times12inline": {
        "math": "<mn>4</mn><mo>⋅</mo> <mn>3</mn><mo>=</mo><mn>12</mn>"
    },
    "timesx": "<span class='symbol'><math class='inline-math'><mo>&times;</mo></math></span>",
    "times*": "<span class='symbol'><math class='inline-math'><mo>*</mo></math></span>",
    "2times3": {
        "math": "<mn>2</mn><mo>⋅</mo> <mn>3</mn>"
    },
    "3times2": {
        "math": "<mn>3</mn><mo>⋅</mo> <mn>2</mn>"
    },    
    "timescommute": {
        "dmath": """<mtable>
        <mtr>
          <mtd>
            <mn>3</mn><mo>⋅</mo><mn>4</mn>
          </mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>12</mn><mo>=</mo><mn>4</mn><mo>⋅</mo><mn>3</mn></mtd>
        </mtr>
        <mtr> <mtd></mtd></mtr>
        <mtr>
          <mtd>
            <mn>2</mn><mo>⋅</mo><mn>7</mn>
          </mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>14</mn><mo>=</mo><mn>7</mn><mo>⋅</mo><mn>2</mn></mtd>
        </mtr>
        <mtr><mtd></mtd></mtr>
        <mtr>
          <mtd>
            <mn>8</mn><mo>⋅</mo><mn>10</mn>
          </mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>80</mn><mo>=</mo><mn>10</mn><mo>⋅</mo><mn>8</mn></mtd>
        </mtr>
      </mtable>""",
    },
    "2times4": {
        "math": "<mn>2</mn><mo>⋅</mo><mn>4</mn>"
    },
    "2times4eq8": {
        "dmath": "<mn>2</mn><mo>⋅</mo><mn>4</mn><mo>=</mo><mn>8</mn>",
    }, 
    "4times2eq8": {
        "dmath": "<mn>4</mn><mo>⋅</mo><mn>2</mn><mo>=</mo><mn>8</mn>",
    },
    "repeatedaddition": {
        "dmath": """<mtable>
        <mtr>
        <mtd>
          <mn>4</mn><mo>+</mo><mn>4</mn><mo>+</mo><mn>4</mn>
        </mtd>
        <mtd><mo>=</mo></mtd>
        <mtd><mn>4</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>3</mn><mo>+</mo><mn>3</mn><mo>+</mo><mn>3</mn><mo>+</mo><mn>3</mn></mtd>
        </mtr>
        <mtr><mtd></mtd></mtr>
        <mtr>
          <mtd>
            <mn>8</mn><mo>+</mo><mn>8</mn>
          </mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>8</mn><mo>⋅</mo><mn>2</mn><mo>=</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn></mtd>
        </mtr>
        <mtr><mtd></mtd></mtr>
        <mtr>
          <mtd><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn><mo>+</mo><mn>1</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd><mn>1</mn><mo>⋅</mo><mn>5</mn><mo>=</mo><mn>5</mn></mtd>
        </mtr>
      </mtable>""",
    },
    "2plus2plus2": {
        "math": "<mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn>"
    },
    "2times3eq2plus2plus2": {
        "math": "<mn>2</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>2</mn><mo>+</mo><mn>2</mn><mo>+</mo><mn>2</mn>"
    },    
    "3plus3": {
        "math": "<mn>3</mn><mo>+</mo><mn>3</mn>"
    },
    "2times3eq3plus3": {
        "math": "<mn>2</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>3</mn><mo>+</mo><mn>3</mn>"
    },
    "2times4lang": '<p style="text-align: center;"> <math><mn>2</mn><mo>⋅</mo><mn>4</mn></math> betyr å vandre 2 plasser <span class="slant">mot høyre</span>, 4 ganger.</p>',
    "4times2lang": '<p style="text-align: center;"> <math><mn>4</mn><mo>⋅</mo><mn>2</mn></math> betyr å vandre 4 plasser <span class="slant">mot høyre</span>, 2 ganger.</p>',    
    "2times3lang": {
        "dmath": "<mtext>''2 ganger 3''</mtext><mo>=</mo><mn>3</mn><mo>+</mo><mn>3</mn>",
    },
    "amounttimes": "<h3 class='subsection-title' id='amounttimes'>Ganging av mengder</h3>",
    "amounttimesref": '<a href="#amounttimes">Her</a>',
    "2times3alt": {
        "math": "<mn>2</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>1</mn><mo>+</mo><mn>5</mn>"
    },
    "divsym": "<span class='symbol'><math class='inline-math'><mo>=</mo></math></span>",
    "12div3": {
        "math": "<mn>12</mn><mo> :</mo><mn>3</mn>"
    },
    "divinterpret": """    <ul>
      <li> <b>Inndeling av mengder</b>
      <math class="display-math">  <mn>12</mn><mo> :</mo><mn>3</mn> <mo>=</mo> <mtext>''Antallet i hver gruppe når 12 deles inn i 3 like store grupper'' </mtext> </math> </li>
      <li> <b>Antall ganger et tall går på et annet</b>
      <math class="display-math">  <mn>12</mn><mo> :</mo><mn>3</mn><mo>=</mo><mtext>''Antall ganger 3 går på 12''</mtext>  </math> </li>
      <li> <b> Omvendt operasjon av multiplikasjon </b>
      <math class="display-math">  <mn>12</mn><mo> :</mo><mn>3</mn><mo>=</mo><mtext>''Tallet man må gange 3 med for å få 12''</mtext>  </math> </li>
    </ul> """,
    "12div3display": {
        "dmath": "<mn>12</mn><mo> :</mo><mn>3</mn><mo>=</mo><mn>4</mn>",
    },
    "slash": "<span class='symbol'><math class='inline-math'><mo>/</mo></math></span>",
    "12div3eq4": {
        "math": "<mn>12</mn><mo> :</mo><mn>3</mn><mo>=</mo><mn>4</mn>"
    },
    "4times3eq12": {
        "math": "<mn>4</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>12</mn>"
    },
    "6times3eq18": {
        "math": "<mn>6</mn><mo>⋅</mo><mn>3</mn><mo>=</mo><mn>18</mn>"
    },
    "6times3": {
        "dmath": """      <mtable>
        <mtr>
          <mtd> <mn>18</mn><mo> : </mo><mn>6</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd>3</mtd>
        </mtr>
        <mtr>
          <mtd> <mn>18</mn><mo> : </mo><mn>3</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd>6</mtd>
        </mtr>
      </mtable>""",
    },
    "5times7eq35": {
        "math": "<mn>5</mn><mo>⋅</mo><mn>7</mn><mo>=</mo><mn>35</mn>"
    },
    "5times7": {
        "dmath": """<mtable>
        <mtr>
          <mtd> <mn>35</mn><mo> : </mo><mn>5</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd>7</mtd>
        </mtr>
        <mtr>
          <mtd> <mn>35</mn><mo> : </mo><mn>7</mn></mtd>
          <mtd><mo>=</mo></mtd>
          <mtd>5</mtd>
        </mtr>
      </mtable>""",
    }
}


